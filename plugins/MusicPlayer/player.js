...
exports.play = {
		...
		responseChannel.send( wrap('Searching...')).then(response => {
...
function executeQueue(client, msg, queue) {
		...
		if (queue.length === 0) {
			responseChannel.send( wrap('Playback finished.'));
		...
		new Promise((resolve, reject) => {
			...
			// Play the video.
			responseChannel.send( generateResultEmbed(video)).then((cur) => {
